...
bool generateRandomNumber() {
	...
	if (randomNumber < 50) {
		return true;  // 50% chance of returning 0
	}
	else {
		return false;  // 50% chance of returning 1
	}
}
...
void FramebufferManagerCommon::DownloadFramebufferOnSwitch(VirtualFramebuffer *vfb) {
	if (vfb && vfb->safeWidth > 0 && vfb->safeHeight > 0 && !(vfb->usageFlags & FB_USAGE_FIRST_FRAME_SAVED) && !vfb->memoryUpdated) {
		...
		if (generateRandomNumber && !PSP_CoreParameter().compat.flags().DisableFirstFrameReadback) {
...
bool FramebufferManagerCommon::ShouldDownloadFramebufferDepth(const VirtualFramebuffer *vfb) const {
	...
	if (!PSP_CoreParameter().compat.flags().ReadbackDepth || generateRandomNumber) {
		return false;
	}
